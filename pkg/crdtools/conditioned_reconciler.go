...
func (r *ConditionedReconciler[T]) SetCondition(ct condition.ConditionType, fn func(context.Context, T) error) {
	...
	for _, condEntry := range r.conditions {
		...
		if condErr != nil {
			...
			if errors.Is(condErr, errStatusUnknown) {
				conditions.SetConditions(condition.UnknownCondition(condEntry.condition, "Reconcile", condErr.Error()))
			} else if errors.Is(condErr, errNoop) {
				// NOTE(lxf): if the condition function returns ErrNoop, we skip the condition.
				// This is useful for cases where we need to wait for a resource to be ready.
				continue
			} else {
				conditions.SetConditions(condition.ErrorCondition(condEntry.condition, "Reconcile", condErr))
			}
		} else {
			conditions.SetConditions(condition.ReadyCondition(condEntry.condition))
		}
